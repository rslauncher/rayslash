...
Phase 0, Phase 1, Phase 2, the first Phase 3 desktop app launcher pass, the Phase 3 launch-stdio cleanup, Phase 4, Phase 5, Phase 6, Phase 7 settings/customization, Phase 8 learned-ranking/aliases, Phase 9 refactoring/internal-boundaries, and Phase 10 packaging/Linux-integration work are complete. The first Phase 12 utility-provider pass now has default-enabled explicit default-browser `search`, configurable additional web search engines, local unit conversion, currency conversion through Frankfurter pair rates, time lookup through Open-Meteo place/timezone metadata, reboot/shutdown/logout actions, timer/reminder notifications, and generated new-app flair state for newly discovered desktop apps. Phase 10 now includes a packaging inventory, AppStream/metainfo metadata, metadata validation, GitHub Actions CI, Fedora RPM and Arch/AUR recipes, DEB and AppImage builders, direct Flatpak bundles, and tag-driven x86_64/ARM64 releases with one checksum manifest. Phase 9 split core app/search/icon/calculator modules, UI callback/helper modules, reusable Slint components, and core integration fixtures while preserving current behavior. Phase 7 now has an in-launcher settings panel with an unframed gear header button, folder sources with a native folder picker, alias and additional search-engine row editors, current provider toggles including aliases and Phase 12 providers, an optional alternate folder opener with an installed-app picker filtered to apps that declare `inode/directory` or use folder-relevant categories such as file managers, terminals, and IDEs, max result count, dark/dim/light theme controls, density controls, learned-ranking controls, autosave for settings changes, Esc-to-return from settings to the main launcher view, and aligned card-based diagnostics including resolved app icon count. Phase 8 has local learned ranking plus aliases/quick links for URLs, files, folders, and explicit no-shell commands. Desktop app discovery now filters unavailable desktop entries with missing `TryExec` targets, missing `Exec` programs when no `TryExec` is present, incompatible `OnlyShowIn`/`NotShowIn` metadata, and supports localized labels, keywords, desktop actions, `StartupWMClass`, and `DBusActivatable`; Flatpak builds additionally discover host exports and run external actions through `flatpak-spawn`. Phase 6 has local install documentation, a Linux desktop entry template, launcher/window icon polish, theme-aware app icon lookup, real launcher UI/UX fixes for shortcut placement, scrollable results, keyboard/mouse selection, opening selection state, click-outside hiding, header icon alignment, app icon display quality, Ctrl-held folder opener row icon switching, and opt-in performance profiling. The previous Phase 1 show-animation polish item is considered good enough for v1 after manual testing.
...
The latest utility/settings correction moves remote time and currency resolution out of the per-keystroke UI path, waits for the provider-declared debounce interval, and discards stale query generations. Time lookup canonicalizes punctuation-free names such as `washington dc`, prevents `america` from resolving to the same-named Dutch village, suppresses unrelated results, and expands countries through the local IANA timezone database into rows grouped by current UTC offset. Time tooltips allow three wrapped lines and clamp their screen position inside the result viewport. The permanent first `Web Search` template uses keyword `search` and a user-editable Google `%s` URL for every browser; it can be disabled but not removed. Search-engine settings use compact favicon/name/URL/keyword rows with centered keyword flairs, toggles, and Edit actions. Alias settings use matching compact rows plus a modal editor for explicit type selection. Both Add/Edit flows use explicit-save in-window editors that survive application focus loss; Delete is available only there for existing removable items. Alias and Web Search configuration sections appear only while their packages are installed, while retained rows remain in `config.toml` after module removal. The alternate folder-opener picker now shows real desktop `Exec` commands rather than internal DBus `gio launch` commands. Max results is incorporated into the Launcher content row, and the main launcher content shares the Settings header's top alignment. Non-default engines prefer the largest favicon declared by the site and render the cached image with smooth interpolation in both settings and module result rows, retaining a magnifying-glass/keyword fallback when no icon is available. Empty launcher queries skip optional module-host execution, and the daemon-style Slint event loop keeps the resident process alive after hiding its only window so subsequent shortcut toggles use fast IPC instead of cold startup. System actions support fuzzy partial queries and additional aliases, run immediately unless a delay is explicit, and participate in learned ranking without tests executing the destructive commands.

The current UI supports dark, dim, and light themes; keyboard and mouse selection; a bounded virtualized result list; deferred/cached icons; active search pills; provider-aware empty/error rows; settings editors; tooltips; learned ranking; and new-app flair. Enter or click activates typed app, folder, URL, command, timer, calculation, conversion, and time actions without a shell. Ctrl selects the configured alternate folder opener. A versioned desktop-app catalog loads before first show and reconciles in the background; opening Settings no longer performs a synchronous full scan. Local results publish immediately, while one generation-aware scheduler merges only relevant module results later. Event-driven callbacks replace result/module/registry/app polling. Config backups are skipped for unchanged content and capped at five; ranking and app state are persisted through one background writer. `rayslash toggle` contacts the resident process over a Unix domain socket and shows or hides the launcher.
...
- Settings autosave rewrites known config fields atomically and does not preserve unknown fields, comments, ordering, or formatting in-place. It skips identical content, retains at most five timestamped backups, and blocks saves after startup config read/parse failures.
...
- Desktop app startup uses a persistent versioned catalog and reconciles standard application directories in the background.
...
- Performance profiling is opt-in through `RAYSLASH_PROFILE=1`; it covers corrected startup boundaries, first redraw request, local and module result refresh, and IPC handling. The ignored `crates/rayslash-core/tests/performance.rs` probes cover synthetic search distributions, live discovery, action dispatch, individual module cold/warm execution and memory, and installed-module routing. [PERFORMANCE.md](PERFORMANCE.md) records the 2026-07-24 implementation, before/after results, closed roadmap, and remaining measurement gaps.
