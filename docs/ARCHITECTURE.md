...
Semantic-version tags drive native x86_64 and ARM64 RPM, DEB, AppImage, and Flatpak builds. Every format includes the same digest-pinned module-host executable, so each architecture/format combination is one download. AppImage resolves that host relative to its mounted executable. Flatpak desktop discovery adds host desktop/icon exports, skips sandbox-local `TryExec` rejection for those verified host entries, and wraps external launcher actions with `flatpak-spawn --host`. Release publication accepts exactly eight user-facing binaries and one checksum manifest, keeping source/debug packages and build intermediates out of the public asset list.
...
45. Web search is represented by configured URL templates only. The first permanent template is `Web Search`, uses keyword `search`, and defaults to `https://www.google.com/search?q=%s`. It can be disabled and its URL can be edited, but it cannot be removed. Add and Edit share an explicit-save modal editor, while legacy incomplete rows remain inactive until completed. Alias rows use the same explicit-save editor lifecycle for URL, file, folder, command, and auto-detected targets. Non-default engine favicons are fetched into the XDG cache in the background, prefer the largest site-declared icon, try common high-resolution icon paths before the legacy root favicon, are normalized to PNG, and are rendered with smooth interpolation in settings and matching module result rows. The same muted-accent technique used by the alternate folder opener colors the active search pill. The permanent default retains its neutral magnifying-glass treatment. All web-search activation opens the rendered URL through `xdg-open`, normally handing it to an existing browser as a new tab without browser-family-specific command handling.
...
51. Ctrl+Enter and Ctrl+click activate a selected folder result by asking core to spawn the configured alternate folder opener. The configured value is parsed into direct program/argument values without a shell. The installed-app picker derives this value from the desktop entry's `Exec` field, preserving static arguments while removing field codes; this avoids exposing `gio launch` used internally for DBus activation. Most commands receive the folder path as their final argument; the default `xdg-terminal-exec` is launched with the folder as its working directory and no implicit folder argument.
...
59. The General settings page autosaves folder sources, alias/search rows, core Apps/Folders toggles, alternate folder opener, learned ranking, theme, density, result limits, and tooltips to `config.toml`. Alias and Web Search configuration sections are shown only when their packages are installed; hiding a removed module's section does not erase its retained `config.toml` rows. The Modules page loads Installed/Official/Community metadata from the verified registry and performs asynchronous install, restore, repair, update, enable, disable, removal, and separate data-removal operations. Module config changes are atomic, permission expansion requires explicit approval, and successful state changes refresh the active query and settings-section visibility. General settings saves back up an existing `config.toml`; writes remain blocked after unsafe config fallback so user-authored configuration is never overwritten.
