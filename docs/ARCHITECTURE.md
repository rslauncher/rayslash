...
- `crates/rayslash-core/src/modules/descriptors.rs`: the seven official built-in module descriptors and descriptor validation.
...
The launcher panel is a compact fixed stack: header, search field, single-line shortcut/status hint, bounded result viewport, and optional result tip text. The right side of the header contains an unframed inline-vector gear Settings button that replaces the old preview label and toggles the settings panel open or closed. Settings open inside the same launcher panel, rather than as a second window. Settings has top-level `General` and `Modules` navigation; Modules has `Installed` and `Official` tabs for the bundled first slice, while the community registry is visibly deferred. Esc closes the settings panel back to the launcher view first; pressing Esc from the launcher view hides the window. Results live inside a Slint `Flickable`, so long app/folder lists scroll within the middle of the panel instead of pushing the hint line out of view. The result viewport uses layout stretch for the remaining launcher height instead of a self-referential fixed height, so rows can remain visible while the model and selection stay independent. Shortcut hint text is split into key/description segments with zero horizontal stretch so each description stays adjacent to its key. Keyboard selection remains clamped in the UI; an empty query starts with no selected row, while a non-empty query with matches selects the first result for quick type-and-Enter use. Up/Down and Tab/Shift+Tab adjust the result viewport when needed so the selected row stays visible. Mouse hover selects/highlights visible rows, including clipped partial rows, without adjusting the result scroll position. Mouse clicks activate rows through the same callback path as Enter. Query changes and IPC show/reset reset selection and scroll position to the top. The UI uses Slint's winit accessor to hide the launcher on `WindowEvent::Focused(false)`, covering ordinary click-outside behavior while suppressing the focus loss caused by the folder picker.
...
12. UI loads `~/.config/rayslash/config.toml`, then loads or creates versioned `~/.config/rayslash/modules.toml`. On first creation, the seven official module entries are seeded from the legacy provider booleans. Once `modules.toml` exists, its official-module entries take precedence and are applied to the runtime provider config; missing official entries still inherit the matching legacy value. A main-config read/parse failure or a module-config read/parse/version failure uses a safe in-memory legacy-derived module view and blocks module writes rather than overwriting the broken file.
...
59. The General settings page autosaves folder sources, alias rows, additional search-engine rows, the core Apps/Folders toggles, alternate folder opener enabled/command, learned ranking enabled/disabled, theme, density, max result count, and tooltip visibility back to `config.toml`. The Modules page lists seven installed official modules—Calculator, Units, Currency, Time, Web Search, Timers, and Aliases—and saves enable/disable changes atomically to `modules.toml`. A successful module toggle is applied immediately, refreshes the active query, and mirrors all seven module states back to the compatibility booleans in `config.toml`, including `providers.utility_actions`; mirror failure is reported without discarding the already-saved module state. Alias rows use `name | keyword | kind | target`; search-engine rows use `on/off | name | keyword | url with %s`. The alternate folder opener picker lists discovered apps that either declare `inode/directory` support or use folder-relevant desktop categories such as `FileManager`, `TerminalEmulator`, or `IDE`. Terminal picks save `xdg-terminal-exec` so folders open as the terminal working directory. Toggles, segmented controls, and picker choices save immediately; text fields save on Enter or focus loss, while multiline alias/search fields save on focus loss. Settings content scrolls inside the launcher panel when it exceeds the visible area, and setting toggles expose full label/description detail after an 800ms hover over the toggle row when tooltips are enabled. General settings values are converted to a validated `Config` before saving. Main settings saves create a timestamped backup of an existing `config.toml` before replacement, unchanged focus-loss saves return without writing or rescanning, and save attempts are blocked if startup fell back to defaults after a config read or parse error. Module toggles are separately blocked when main or module config could not be safely loaded. After a successful save, UI refreshes the current query through the shared result/settings refresh path without restarting the resident process.
...
`modules.toml` uses `version = 1`, contains entries keyed by module ID, and is written with the same atomic temp-file/rename helper as other generated TOML writes. Its schema flattens unknown top-level fields and unknown fields inside each module entry so they survive a load/save round trip; unknown module entries are retained but cannot be toggled as official modules. Parse-corrupt or unsupported versions are never replaced with fallback defaults: the UI falls back to a legacy-derived in-memory view and blocks module writes until the file is fixed and rayslash is restarted.

On the first valid startup without `modules.toml`, rayslash creates it from the seven legacy provider booleans. Once present, official entries in `modules.toml` take precedence at startup; missing official entries are seeded in memory from their legacy boolean. Module toggles update `modules.toml` first and then mirror Calculator, Aliases, Web Search, Units, Currency, Time, and Timers back to `config.toml` compatibility fields. Apps and folders remain core settings and are never inserted as module entries.
